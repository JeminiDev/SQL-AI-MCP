...
export async function createSqlConfig(): Promise<{ config: sql.config, token: string | null, expiresOn: Date | null }> {
  const authType = process.env.AUTH_TYPE || 'azure-ad';
  const trustServerCertificate = process.env.TRUST_SERVER_CERTIFICATE?.toLowerCase() === 'true';
  const connectionTimeout = process.env.CONNECTION_TIMEOUT ? parseInt(process.env.CONNECTION_TIMEOUT, 10) : 30;
  const serverPort = process.env.SERVER_PORT ? parseInt(process.env.SERVER_PORT, 10) : undefined;

  if (authType === 'sql') {
    // SQL Authentication (username/password)
    if (!process.env.SQL_USERNAME || !process.env.SQL_PASSWORD) {
      throw new Error('SQL_USERNAME and SQL_PASSWORD must be set when using AUTH_TYPE=sql');
    }
...
async function ensureSqlConnection() {
  ...
  // For SQL auth, just check if we have a connected pool
  if (authType === 'sql') {
    if (globalSqlPool && globalSqlPool.connected) {
      return;
    }
  } else {
    // For Azure AD auth, check token expiry as well
    if (
      globalSqlPool &&
